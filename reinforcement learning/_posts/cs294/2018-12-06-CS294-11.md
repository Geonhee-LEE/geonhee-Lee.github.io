...
1. 지난 강의: 알고있는 system dynamics(e.g., physics)를 통해 backpropagating(or planning)하여 자체적으로 좋은 action을 선택.
2. 이번 강의: dynamics를 모른다면 어떻게 해야할까?
    1. Fitting global dynamics models ("model-based RL").
    2. Fitting local dynamics models.
3. 다음 강의:
    1. Image들과 같이 높은 차원 observation에 대해 dynamics를 학습.
4. 다다음 강의:
    1. Optimal control의 목표를 가지고 NN policies를 학습하기 위해 optimal control과 policy search 결합.
